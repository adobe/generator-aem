...
const _duplicateCheck = function () {
  let dup = false;
  const parentPath = path.dirname(this.destinationPath());
  const modules = PomUtils.listParentPomModules(this, parentPath);
  _.each(modules, (module) => {
    const yorc = path.join(parentPath, module, '.yo-rc.json');
    if (this.fs.exists(yorc)) {
      dup = this.fs.readJSON(yorc)[this.rootGeneratorName()] !== undefined && module !== path.basename(this.destinationPath());
...
const _findModules = function (type) {
  const modules = [];
  let root = this.destinationPath();
  if (!this.fs.exists(path.join(root, '.yo-rc.json')) || this.fs.readJSON(path.join(root, '.yo-rc.json'))[parentGenerator] === undefined) {
