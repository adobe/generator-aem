 ...
 *
 * @param generator the generator's context
 * @returns {*[]} list of template files
 */
function listTemplates(generator) {
  const files = [];

  let patterns = [generator.templatePath('shared', '**/*'), generator.templatePath('shared', '**/.*')];
  ...
  for (const idx in paths) {
    if (Object.prototype.hasOwnProperty.call(paths, idx)) {
      ...
      files.push({
        src: file,
        dest: generator.destinationPath(generator.relativePath, path.relative(generator.templatePath('shared'), file)),
      });
    }
  }

  if (generator.props.examples) {
    patterns = generator.templatePath('examples', '**/*');
    const paths = globbySync(patterns, { onlyFiles: true });
    for (const idx in paths) {
      if (Object.prototype.hasOwnProperty.call(paths, idx)) {
        const file = paths[idx];
        files.push({
          src: file,
          dest: generator.destinationPath(generator.relativePath, path.relative(generator.templatePath('examples'), file)),
        });
      }
    }
  }
