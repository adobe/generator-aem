...
/**
 * List all of the module paths in the generator's parent pom
 *
 * @param {Generator} the generator context
 * @return {[]|Array} list of modules or an empty list
 */
const listParentPomModules = (generator) => {
  const root = path.dirname(generator.destinationRoot());
  if (!generator.fs.exists(path.join(root, 'pom.xml'))) {
    return [];
  }
  const pom = new XMLParser().parse(generator.fs.read(path.join(root, 'pom.xml')));
