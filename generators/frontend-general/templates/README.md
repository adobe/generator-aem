...
The second part of the ui.frontend module build process leverages the [aem-clientlib-generator](https://www.npmjs.com/package/aem-clientlib-generator) plugin to move the compiled CSS, JS and any resources into the `src/main/content` folder. The `ui.apps` project is responsible for pulling in these output files for inclusion in its build artifact. The aem-clientlib-generator configuration is defined in `clientlib.config.js`. The following client libraries are generated:

* **clientlib-site** - `src/main/content/jcr_root/apps/<app>/clientlibs/clientlib-site`
* **clientlib-dependencies** - `src/main/content/jcr_root/apps/<app>/clientlibs/clientlib-dependencies`
